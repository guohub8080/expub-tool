...
| `depthLaw` | `number` | `0` | 深度分布（等差间距）：`0`=均等；正数相邻间距从 tail→center 递增（tail 密集），负数递减（tail 拉开）。数值 ≈ 每步 gap 相对均等值的递增百分比（`10`≈每步+10%）。scale 跟随位置。负向受 gap 非负限制（约 `−100/(层数−2)`）|
...
- **深度分布（depthLaw，等差间距）**：相邻层间距成等差数列 `gap[i] = base·(1 + i·r)`，`r = depthLaw/100`，`base` 由 `sum(gap)=anchorDistance` 反推。
  - `0`：均等（公差 0）
  - `>0`：gap 递增，tail 侧密集、center 侧拉开
  - `<0`：gap 递减，tail 侧拉开、center 侧密集
  - 数值 ≈ 每步 gap 相对均等值的递增百分比（`10`≈每步 +10%）
  - r 钳制使 gap 非负（负向上限约 `−100/(层数−2)`）
  - scale 跟随位置：`scale = 1 − (1−tailChild.scale)·progress`
