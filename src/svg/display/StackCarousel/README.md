...
| `depthLaw` | `number` | `0` | 深度分布：`0`=完全线性（均匀）；`[-100,0]` 堆积 tail（center 除外），`[0,+100]` 堆积 center（tail 除外）。scale 跟随位置（堆积处变小/变大）。线性混合控制，`±100` 为极端堆积 |
...
- **深度分布（depthLaw，范围 [-100, +100]）**：scale 和位置由同一个 progress 驱动，progress 由「线性分布」与「极端堆积」按 `blend = |depthLaw|/100` 线性混合得到。`x = i/(n−1)`（0=tail, 1=center），`linearProgress = 1−x`，极端分布：`depthLaw<0` 时 center 留 center、其余堆 tail；`depthLaw>0` 时 tail 留 tail、其余堆 center。`progress = (1−blend)·linearProgress + blend·extreme`，`scale = 1 − (1−tailChild.scale)·progress`（scale 跟随位置）。
  - `0`：完全线性，各层均匀
  - `−100`：除 center 外全堆 tail（一堆小卡 + 一个大 center）
  - `+100`：除 tail 外全堆 center（一个小 tail + 一堆大卡）
  - 中间值线性混合，调一点偏一点（无幂次敏感）
