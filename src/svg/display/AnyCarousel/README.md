...
- N+3 个 slot 沿 +angle 排开（首尾各一个 ghost 副本保证无缝循环），外层 `<g>` 用一条 `transformTranslate`（`isAdditive`）整体平移，让 slot 依次滑过中心。
