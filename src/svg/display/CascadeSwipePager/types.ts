...
export interface I_CascadeContent {
  url?: string
  jsx?: ReactNode
}
...
export interface I_CascadeSwipeChildItem {
  /** 右侧把手（手动 w/h/y，靠右） */
...
export interface I_CascadeSwipePagerProps {
  /** SVG viewBox 尺寸（必填） */
  canvasSize: { w: number; h: number }
  /** 整体背景层（渲染在所有卡背后）：url 或 jsx，不传则无背景 */
  canvasBg?: I_CascadeContent
  /** N 个抽拉卡（零高视差叠加）；每卡一个把手 + 一组横向面板（必填） */
  childItems: I_CascadeSwipeChildItem[]
  /** 外间距 */
