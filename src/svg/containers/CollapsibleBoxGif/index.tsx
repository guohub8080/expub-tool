...
/**
 * 坍塌盒子 GIF 版 — 点击后 GIF 播放 N 秒再坍塌，可显示替换图片
 *
 * 在 CollapsibleBox 基础上增加：
 * - 延迟坍塌（等待 GIF 播放）
 * - foreignObject + translate 实现替换图片从屏幕外移入
 *
 * @param children            - 展示内容（通常是 GIF）
 ...
 * @param afterSwitchImgUrl   - 坍塌后显示的替换图片 URL
 * @param gifDuration         - GIF 播放时长（秒），默认 2
 * @param spacing             - 外边距配置
 */
const CollapsibleBoxGif = (props: {
  ...
  return (
    <SectionEx
      {...(isDev ? { 'expubgo-label': 'collapsible-box-gif' } : {})}
