# SnapSlideViewXContainer — 横向吸附滑动视口容器

在 SlideViewXContainer 基础上增加 **scroll snap 吸附效果**，滑动松手后自动对齐到最近的子组件，实现类似轮播的"翻页"体验。
...
与 SlideViewXContainer 的三层结构相同，额外增加：
...
```tsx
<SnapSlideViewXContainer snapAlign="start" items={[...]} />
```

### 反向轮播（从右侧开始）

```tsx
<SnapSlideViewXContainer isReverse items={[...]} />
```

## 注意

- `items` 为空时返回 null
- 需要 scroll snap 的场景用本组件，自由滑动用 SlideViewXContainer
