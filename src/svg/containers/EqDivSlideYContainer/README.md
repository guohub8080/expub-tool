...
| h | number | — | 视口高度基准 |
| isReverse | boolean | false | 是否反向排列（column-reverse，从最后一项开始） |
...
```tsx
<EqDivSlideYContainer w={300} h={450} items={[
  <ImgSvg url={img1} w={300} h={500} />,
  <ImgSvg url={img2} w={300} h={500} />,
]} />
```

h=450 < 图片高度 500，视口只露出 90% 的图片，底部露出下一张的顶部，暗示用户可以继续滑动。

### 反向排列（从底部开始）

```tsx
<EqDivSlideYContainer isReverse w={300} h={500} items={[...]} />
```
...
- 与 EqDivSnapSlideYContainer 的区别：本组件自由滑动，不带吸附效果
