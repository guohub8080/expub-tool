# SnapSlideViewYContainer — 纵向吸附滑动视口容器（抖音效果）

在 SlideViewYContainer 基础上增加 **scroll snap 吸附效果**，滑动松手后自动对齐到最近的子组件，实现类似抖音的"刷视频"体验。

## 原理

与 SlideViewYContainer 相同的 `aspect-ratio` 方案，额外增加 scroll snap：
...
与 SlideViewYContainer 的区别仅在外层额外增加：
...
```tsx
<SnapSlideViewYContainer w={300} h={500} items={[
...
```tsx
<SnapSlideViewYContainer w={300} h={500} isBottomUp items={[...]} />
```

### 吸附到中心

```tsx
<SnapSlideViewYContainer snapAlign="center" w={300} h={500} items={[...]} />
```

### 反向（从最后一页开始）

```tsx
<SnapSlideViewYContainer isReverse w={300} h={500} items={[...]} />
```
...
- 需要自由滑动（不吸附）用 SlideViewYContainer
